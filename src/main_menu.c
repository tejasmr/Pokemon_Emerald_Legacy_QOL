...
static void Task_NewGameBirchSpeech_NormalText(u8);
static void Task_NewGameBirchSpeech_HardText(u8);
...
static void Task_NewGameBirchSpeech_ChooseNuzlocke(u8 taskId)
{
    ...
    switch (nuzlocke)
    {
        ...
        case 0:
            ...
            FlagSet(FLAG_HARD);
            FlagClear(FLAG_NUZLOCKE);
            gTasks[taskId].func = Task_NewGameBirchSpeech_HardText;
            break;
        case 2:
            NewGameBirchSpeech_ClearNuzlockeWindow(3, 1);
            PlaySE(SE_SELECT);
...
static void Task_NewGameBirchSpeech_NormalText(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_AreYouSure_Normal);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_AreYouSure;
}

static void Task_NewGameBirchSpeech_HardText(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_AreYouSure_Hard);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_AreYouSure;
}

static void Task_NewGameBirchSpeech_HardcoreText(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_AreYouSure_Hardcore);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_AreYouSure;
}

static void Task_NewGameBirchSpeech_AreYouSure(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameBirchSpeech_AreYouSureSelection;
    }
}

static void Task_NewGameBirchSpeech_AreYouSureSelection(u8 taskId)
{
...
static void NewGameBirchSpeech_ShowNuzlockeMenu(void)
{
    ...
    PrintMenuTable(3, ARRAY_COUNT(sMenuActions_Nuzlocke), sMenuActions_Nuzlocke);
